...
An object merge with a repeat part and a rest, where the repeat count is
solvable, is underdetermined: the count could be anything from 0 to the member
count, with the rest absorbing the difference. Only the exact (no-rest) shape
solves for the count. Expected: a diagnostic, or a restriction to bound counts.

  $ possum -p 'const({"a": 1, "b": 2, "c": 3}) -> {...({_: _} * N), ...Rest} $ [N, Rest]' -i ''
  [UnsupportedPattern]
  [1]
