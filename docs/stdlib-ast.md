...
Binding power can also model associativity. By making the binding power on either side of an infix operator asymmetric we can dictate if it is left- or right-associative. Decreasing the left binding power makes an operator left-associative, while decreasing the right binding power makes it right-associative.

Assign binding power 2/1 to `+` to make the operation right associative:
...
Assign binding power 1/2 to `+` to make the operation left associative:
...
* `prefix`: Parser for operators that can be placed in front of any operand. The result of `prefix` must be an object, which will become an inner node of the AST. The object must have a `"power": BindingPower` number field for the operator's binding power. If there are no prefix operators to parse, use the `@fail` parser to skip this step. In the following example `prefix` parses either a minus sign or the `not` keyword, and returns a node object with binding power in both cases.

    ```
    prefix =
      ("-" $ {"type": "negate_number", "power": 6}) |
      (word -> "not" $ {"type": "negate_boolean", "power": 5})
    ```
    After a prefix is parsed the prefixed expression is also parsed based on the prefix operator's binding power, and the resulting subtree is added to the prefix node object under the the key `"prefixed"`. For example with the `operand` and `prefix` parsers defined above the expression `"-5"` would parse as:
...
* `postfix`: Parser for operators that can be placed after any operand. The result of `postfix` must be an object, which will become an inner node of the AST. The object must have a `"power": BindingPower` number field for the operator's binding power. If there are no postfix operators to parse, use the `@fail` parser to skip this step. This parser is very similar to `prefix`, but the resulting AST node uses the key `"postfixed"` for the subtree modified by the operator.

#### Tracking source position

Optionally, `ast.with_operator_precedence` can handle source position information for each node in the AST. This requires `operand` to return object nodes, and for all operand/prefix/infix/postfix nodes to include `"startpos"` and `"endpos"` fields. For example, we can wrap each node in a helper `with_offset_pos` which adds the start/end byte offset for each parsed node.
...
Calling `with_offset_pos(prefix)` and `with_offset_pos(operand)` on the appropriate parts of `-500` produces these two nodes:

```
{"type": "negate_number", "power": 6, "startpos": 0, "endpos": 1}
{"type": "number", "value": 500, "startpos": 1, "endpos": 4}
```

When passed to `ast.with_operator_precedence` this produce an AST where the position information correctly accounts for operator nodes encompassing the full source range of its sub-nodes. In this case the source range for the `"negate_number"` node is 0-4, including the prefixed number node.
...
Helper parser for creating tagged operand nodes. Parses `value` and returns an object with a `"type"` and `"value"` field. For example
```
ast.node(int, $"integer")
```
might return
```
...
Helper for creating prefix operator parsers. With this function we can rewrite the `prefix` example above to ensure we match the necessary return format for precedence parsing.

```
prefix =
  ast.prefix_node("-", $"negate_number", $6) |
  ast.prefix_node(word -> "not", $"negate_boolean", $5)
```
...
```
infix =
  ast.infix_node("+", $"add", $3, $4) |
  ast.infix_node("=", $"assign", $2, $1)
```

### `ast.postfix_node(op, Type, BindingPower)`

Helper for creating postfix operator parsers. This parser has the same behavior as `ast.prefix_node`.

### `ast.with_offset_pos(node)`

Adds `"startpos"` and `"endpos"` fields to a node, indicating the node's position in the input source. The position is a number indicating number of bytes from the start of the source.

### `ast.with_line_pos(node)`

Adds `"startpos"` and `"endpos"` fields to a node, indicating the node's position in the input source. The position is an object with `"line"` and `"offset"` fields, indicating the line number in source and number of bytes from the start of the line.
