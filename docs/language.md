...
This list is not exhaustive. Pattern matching in Possum is intended to be maximally flexible, but with some notable limitations:
  ...
  - Function calls in patterns are evaluated, not solved: the callee and every argument must be bound values when the call runs. A function variable with no binding occurrence anywhere in the pattern is a compile error.
