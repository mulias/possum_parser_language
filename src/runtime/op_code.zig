...
pub const OpCode = enum(u8) {
    ...
    MatchBind,
    MatchCast,
    // The claim set: an exclusive-claim search over object members backed
    // by a dyn array in a match register, shared by search pairs, object
    // repeats, and object-merge repeats. MatchClaimSeed starts the array
    // from a const-key-list constant (possibly empty); MatchClaimScan finds
    // the first member whose key is not yet in the array (advancing a
    // per-pair cursor); MatchClaimKey probes a popped known key the same
    // way; MatchClaimAdd appends a matched key; MatchClaimDoneCount exits the
    // group loop once the array reaches a count register's target size (the
    // whole member count for an object repeat, or a count-derived size for an
    // inexact object-merge repeat); MatchClaimCount derives an object-merge
    // repeat's group count from the seeded member count; MatchClaimObject
    // claims a popped evaluated object part member-by-member against src;
    // MatchClaimRest builds the fresh rest object of the members left
    // unclaimed.
    MatchClaimAdd,
    MatchClaimCount,
    MatchClaimDoneCount,
    ...
    MatchCmp,
    MatchCount,
    // Pop the evaluated repeat count product; write the target claim size
    // (the claim array's current length + count * pair_len) into a match
    // register for MatchClaimDoneCount.
    MatchCountLoad,
    MatchBound,
    MatchDivideEval,
    MatchElem,
    ...
    pub fn stackEffect(self: OpCode) StackEffect {
        return switch (self) {
            ...
            // residual part), MatchMergeEval (an untyped merge's evaluated
            // parts), MatchClaimKey (the probed object key).
            .MatchEval,
            .MatchClaimKey,
            .MatchClaimObject,
            .MatchCountLoad,
            ...
            .MatchRepeatNext,
            // The count-driven group loop exits forward once the claim array
            // reaches its target size; a genuine success branch like
            // MatchRepeatNext.
            .MatchClaimDoneCount,
            => .{ .branch = .{
    ...
    // CaptureLocal, and container construction inside
    ...
    pub fn rcEffect(self: OpCode) ?RcEffect {
        return switch (self) {
            ...
            .MatchBound,
            .MatchCast,
            // Creates a fresh claim array (MatchClaimSeed) or appends a
            // matched key into it (MatchClaimAdd, retaining in the
            // dispatch) or projects a member's key/value into registers
            // (MatchClaimScan, retaining in the dispatch); the loop-done
            // test moves nothing (MatchClaimDoneCount).
            .MatchClaimAdd,
            .MatchClaimCount,
            .MatchClaimDoneCount,
            .MatchClaimRest,
            ...
            .MatchClaimKey,
            // Pops and releases the evaluated object part; the claimed keys
            // are retained into the claim array in the dispatch.
            .MatchClaimObject,
            ...
            .MatchSubtractEval,
            // Pops and releases the evaluated merge parts; the residual
            // (number, or a fresh string/array/object) moves into the
            // destination register in the dispatch, releasing the previous.
            .MatchMergeEval,
            // Pops and releases the evaluated count factor; the numeric
            // residual written into the destination is not a Dyn handle.
            .MatchDivideEval,
            // Pops and releases the evaluated count product; the numeric
            // target written into the destination is not a Dyn handle.
            .MatchCountLoad,
    ...
    pub fn disassemble(self: OpCode, chunk: *Chunk, vm: VM, module: Module, writer: *Writer, offset: usize) !usize {
        return switch (self) {
            ...
            .MatchIterInit => self.registerInstruction(chunk, writer, offset),
            .MatchClaimAdd => self.matchRepeatInstruction(chunk, writer, offset),
            .MatchClaimScan => self.matchClaimScanInstruction(chunk, writer, offset),
            .MatchClaimKey => self.matchClaimKeyInstruction(chunk, writer, offset),
            .MatchClaimSeed => self.matchClaimSeedInstruction(chunk, vm, module, writer, offset),
            .MatchClaimCount => self.matchClaimCountInstruction(chunk, writer, offset),
            .MatchClaimObject => self.matchClaimObjectInstruction(chunk, writer, offset),
            .MatchClaimRest => self.matchClaimRestInstruction(chunk, writer, offset),
            .MatchClaimDoneCount => self.matchClaimDoneCountInstruction(chunk, writer, offset),
            .MatchCountLoad => self.matchCountLoadInstruction(chunk, writer, offset),
            ...
            .MatchMergeEval => self.matchMergeEvalInstruction(chunk, writer, offset),
    ...
    fn matchMergeEvalInstruction(self: OpCode, chunk: *Chunk, writer: *Writer, offset: usize) !usize {
        const dst = chunk.read(offset + 1);
        const src = chunk.read(offset + 2);
        const back = chunk.read(offset + 3);
        if (back != 0) {
            try writer.print("{s} r{} <- r{} - <pop> (back)\n", .{ @tagName(self), dst, src });
        } else {
            try writer.print("{s} r{} <- r{} - <pop>\n", .{ @tagName(self), dst, src });
        }
        return offset + 4;
    ...
    fn matchClaimScanInstruction(self: OpCode, chunk: *Chunk, writer: *Writer, offset: usize) !usize {
        ...
        const claim = chunk.read(offset + 5);
        try writer.print("{s} key=r{} val=r{} src=r{} cursor=r{} claims=r{}\n", .{
            @tagName(self), key_dst, val_dst, src, cursor, claim,
        });
        return offset + 6;
    }

    fn matchClaimKeyInstruction(self: OpCode, chunk: *Chunk, writer: *Writer, offset: usize) !usize {
        const key_dst = chunk.read(offset + 1);
        const val_dst = chunk.read(offset + 2);
        const src = chunk.read(offset + 3);
        const claim = chunk.read(offset + 4);
        try writer.print("{s} key=r{} val=r{} src=r{} claims=r{}\n", .{
            @tagName(self), key_dst, val_dst, src, claim,
        });
        return offset + 5;
    }

    fn matchClaimDoneCountInstruction(self: OpCode, chunk: *Chunk, writer: *Writer, offset: usize) !usize {
        const claim = chunk.read(offset + 1);
        const count = chunk.read(offset + 2);
        const jump = (@as(u16, @intCast(chunk.read(offset + 3))) << 8) | chunk.read(offset + 4);
        const target = offset + 5 + jump;
        try writer.print("{s} claims=r{} count=r{} done->{}\n", .{ @tagName(self), claim, count, target });
        return offset + 5;
    }

    fn matchClaimSeedInstruction(self: OpCode, chunk: *Chunk, vm: VM, module: Module, writer: *Writer, offset: usize) !usize {
        const dst = chunk.read(offset + 1);
        const constantIdx = (@as(usize, @intCast(chunk.read(offset + 2))) << 8) | chunk.read(offset + 3);
        var keys = module.getConstant(constantIdx);
        try writer.print("{s} r{} <- ", .{ @tagName(self), dst });
        try keys.print(vm, writer);
        try writer.print("\n", .{});
        return offset + 4;
    }

    fn matchClaimCountInstruction(self: OpCode, chunk: *Chunk, writer: *Writer, offset: usize) !usize {
        const src = chunk.read(offset + 1);
        const pair_len = chunk.read(offset + 2);
        const seed = chunk.read(offset + 3);
        const count_dst = chunk.read(offset + 4);
        try writer.print("{s} n=r{} <- (members(r{}) - {}) / {}\n", .{ @tagName(self), count_dst, src, seed, pair_len });
        return offset + 5;
    }

    fn matchClaimObjectInstruction(self: OpCode, chunk: *Chunk, writer: *Writer, offset: usize) !usize {
        const claim = chunk.read(offset + 1);
        const src = chunk.read(offset + 2);
        try writer.print("{s} claims=r{} <- members(<pop>) in r{}\n", .{ @tagName(self), claim, src });
        return offset + 3;
    }
    fn matchClaimRestInstruction(self: OpCode, chunk: *Chunk, writer: *Writer, offset: usize) !usize {
        const dst = chunk.read(offset + 1);
        const src = chunk.read(offset + 2);
        const claim = chunk.read(offset + 3);
        try writer.print("{s} r{} <- r{} \\ claims=r{}\n", .{ @tagName(self), dst, src, claim });
        return offset + 4;
    }

    fn matchCountLoadInstruction(self: OpCode, chunk: *Chunk, writer: *Writer, offset: usize) !usize {
        const dst = chunk.read(offset + 1);
        const pair_len = chunk.read(offset + 2);
        const claim = chunk.read(offset + 3);
        try writer.print("{s} r{} <- len(r{}) + <pop> * {}\n", .{ @tagName(self), dst, claim, pair_len });
        return offset + 4;
    }
