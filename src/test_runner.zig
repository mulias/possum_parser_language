//! Default test runner for unit tests, vendored from zig 0.16.0's
//! lib/compiler/test_runner.zig with one fix: the fuzz error path called
//! std.debug.writeStackTrace with a *std.builtin.StackTrace, which no longer
//! compiles (the two StackTrace types diverged in 0.16); it now calls
//! std.debug.writeErrorReturnTrace. Without the fix, `zig build test --fuzz`
//! fails to build. Delete this file and the test_runner wiring in build.zig
//! once the upstream runner is fixed.
