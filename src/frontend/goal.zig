...
fn lowerObjectMerge(
    ...
    for (parts, 0..) |part, i| {
        switch (part.node) {
            ...
            // A repeat part claims count-many disjoint member groups. Only
            // one is allowed, its base must be an object chunk, and it does
            // not coexist with a search pair in v1 (the claim array unifies
            // the const-key seed and the repeat's claims, not a third source).
            .repeat => {
                if (has_repeat) return false;
                var base = part;
                while (base.node == .repeat) base = base.node.repeat.left;
                if (base.node != .object) return false;
                has_repeat = true;
            },
            else => return false,
        }
    }
    if (has_repeat and has_search_pair) return false;

    try self.pushConstraint(constraints, .{ .is_type = .{
        .place = place,
        .ty = .object,
    } }, region);
    // A repeat part makes the member count dynamic, so no keys_exact even
    // without a rest: exactness comes from the claim_members divisibility
    // and coverage. keys_min still pins the const-key minimum.
    if (slack == null and !has_repeat) {
        try self.pushConstraint(constraints, .{ .keys_exact = .{
            .place = place,
    ...
    for (parts) |part| {
        switch (part.node) {
            ...
            .repeat => {
                ...
                try self.pushConstraint(constraints, .{ .claim_members = .{
                    .place = place,
                    .pattern = try self.patternPart(base),
                    .count_factors = count_factors,
                    .solvable_index = null,
...
fn printConstraint(
    ...
    switch (constraint.kind) {
        ...
        .claim_members => |c| {
            try writer.print("(claim_members %{d}", .{c.place});
            if (c.exact) try writer.writeAll(" exact");
            if (c.solvable_index) |index| try writer.print(" solvable={d}", .{index});
            try writer.writeAll("\n");
            try printIndent(writer, indent + 1);
            try writer.writeAll("pattern: ");
