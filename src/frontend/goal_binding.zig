...
const Analyzer = struct {
    ...
    fn collectEvalReads(self: *Analyzer, constraint: Ast.Constraint, set: *SlotSet) void {
        switch (constraint.kind) {
            ...
            .claim_members => |claim| {
                self.collectPartEvalReads(claim.pattern, set);
                for (claim.count_factors.items) |factor| self.collectPartEvalReads(factor, set);
            },
            .search_key => |search| {
    ...
    fn classifyConstraint(
        ...
        switch (constraint.kind) {
            ...
            .claim_members => |*claim| {
                try self.classifyPart(env, bindable, &claim.pattern, region);
                for (claim.count_factors.items) |*factor| {
                    try self.classifyPart(env, bindable, factor, region);
                }
                claim.solvable_index = try self.repeatSolvableIndex(claim.count_factors.items, region);
            },
            .search_key => |search| {
    ...
    fn collectBindable(self: *Analyzer, constraints: []const Ast.Constraint, set: *SlotSet) void {
        for (constraints) |constraint| {
            switch (constraint.kind) {
                ...
                .claim_members => |claim| {
                    self.collectBindablePart(claim.pattern, set);
                    for (claim.count_factors.items) |factor| self.collectBindablePart(factor, set);
                },
                .search_key => |search| {
    ...
    fn collectPatternSlots(self: *Analyzer, constraints: []const Ast.Constraint, set: *SlotSet) void {
        for (constraints) |constraint| {
            switch (constraint.kind) {
                ...
                .claim_members => |claim| {
                    self.collectPatternPartSlots(claim.pattern, set);
                    for (claim.count_factors.items) |factor| self.collectPatternPartSlots(factor, set);
                },
                .search_key => |search| {
    ...
    fn verifyConstraints(self: *const Verifier, constraints: []const Ast.Constraint) void {
        for (constraints) |constraint| {
            switch (constraint.kind) {
                ...
                .claim_members => |claim| {
                    self.verifyPart(claim.pattern);
                    for (claim.count_factors.items) |factor| self.verifyPart(factor);
                },
                .search_key => |search| {
