...
pub const Constraint = struct {
    ...
    pub const Kind = union(enum) {
        ...
        // An object merge's repeat part (`{...(chunk * count), ...Rest}`):
        // claim count-many disjoint groups of the object's members, each
        // matching the chunk sub-pattern, into a claim array. The remaining
        // members feed a members_rest place. count_factors multiply like
        // solve_repeat's; solvable_index (filled by binding analysis) names
        // the one unbound factor the count solve binds. exact is true when
        // no rest part follows: the member count is then determined by
        // divisibility and coverage rather than a keys_exact test.
        claim_members: struct {
            place: PlaceId,
            pattern: Part,
            count_factors: ArrayList(Part),
            solvable_index: ?u32,
