...
parser_declarations: StringHashMap(*Parser.Declaration),
value_declarations: StringHashMap(*Value.Declaration),
main: ?*Ast.Parser.RNode,
...
pub fn addParserDeclaration(self: *Ast, decl: *Parser.Declaration) !void {
    try self.parser_declarations.put(self.arena.allocator(), decl.ident.name, decl);
}

pub fn addValueDeclaration(self: *Ast, decl: *Value.Declaration) !void {
    try self.value_declarations.put(self.arena.allocator(), decl.ident.name, decl);
}
...
pub const Value = struct {
    ...
    pub const Node = union(NodeType) {
        ...
        number_float: f64,
        number_string: NumberString,
        number_subtract: struct { left: *Value.RNode, right: *Value.RNode },
        object: ArrayList(ObjectPair),
        repeat: struct { left: *Value.RNode, right: *Pattern.RNode },
        string: []const u8,
...
pub const Pattern = struct {
    ...
    pub const Node = union(NodeType) {
        ...
        number_float: f64,
        number_string: NumberString,
        number_subtract: struct { left: *Pattern.RNode, right: *Pattern.RNode },
        object: ArrayList(ObjectPair),
        range: Range,
