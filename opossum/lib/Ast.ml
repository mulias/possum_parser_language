...
(* Permissive Program AST
   This part of the AST is produced by the ProgramParser and represents a
   program which is at least minimally syntactically correct.
*)

(* A list of parsers interspersed with infix operators. Does not
   impose certian requirements, such as that a sequence of `And infixes must be
...
(* A program which parses structured text into JSON. Does not require that
   there is exactly one main parser to start at.
*)
type permissive_program = [ `Program of permissive_parser list ]
[@@deriving show]

(* AstTransformer Intermediary Grouping
   In order to transform the permissive parser AST to the program AST we need
   to split the permissive parser step lists into sub lists so that the steps
   are grouped into a sequence, or are not part of a sequence.
*)
...
(* Either a sequence of parsers ending with a return step, or a parser which
   is not inside of a sequence.
*)
and parser_group =
...
(* Program AST
   This AST is produced by the AstTransformer, which iterates over the tree to
   build a more strict version which can be evaluated.
*)
